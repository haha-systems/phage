...
| WAL write-path optimization evidence | Active / macOS fallback documented | The S3 evidence note records S1/S2 before/after persisted write rows for macOS POSIX fallback, with roughly 14–15% higher write throughput across batch sizes `1/16/64`; Linux `io_uring` post-optimization evidence remains deferred to S4. See [WAL write-path optimization evidence](benchmarks/2026-05-18-wal-write-path-optimization.md). |
